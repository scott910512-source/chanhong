...
1. **파란 요약 카드** — 총 자산, 평가손익 / 수익률, 오늘 변동
2. **매매 안내** — 국가·섹터·종목 규칙이 사실상 같은 매매를 가리키는 일이 많아
   종목 단위로 합쳐 한 줄씩. 기본 4줄, 나머지는 "n건 더 보기".
   헤더 종 아이콘에 건수 배지가 뜨고 누르면 이 목록으로 갑니다
3. **국가별 비중** — 도넛 + `현재 비중 / 목표 비중` 표, 항목마다 `!`(많음·적음) `✓`(적정)
4. **섹터별 비중** — 가로 막대(목표 위치에 눈금) + 같은 형식의 표
5. **보유 종목** — 종목명·티커 / 보유 수량 / 평균 단가 / 현재 가치 / 평가손익(수익률).
   정렬 `금액순 · 수익률순 · 이름순`

**아직 하나도 없는 국가·섹터도 목표를 걸면 판정합니다.** 삼성전자 1주만 있고 미국
주식이 없으면 `미국 0.0% / 목표 50%` 가 점선 표시로 뜨고 매수 안내가 나갑니다.
새로 시작하면 **한국·미국이 각 50% 로 기본**으로 깔려 있고, 설정에서
`+ 국가 추가` / `+ 섹터 추가` 로 안 가진 항목도 얼마든지 추가할 수 있습니다.

국가·섹터 카드에는 **`금액 기준` / `수량 기준`** 토글이 있습니다. 수량 기준은
보유 주수의 비율이고, 적정·초과 판정은 설정에 걸어둔 기준(보통 비중 %)으로 합니다.

**컴퓨터(900px 이상)** — 요약·안내를 가로로 펼치고 아래를 2단으로,
**왼쪽에 국가·섹터 현황, 오른쪽에 보유 종목**. 탭바는 상단으로 올라갑니다.

**관리** (오른쪽 위 `관리` 버튼 또는 아래 탭) — 추가·수정·삭제를 여기서 다 합니다.

- 종목을 누르면 매매 내역이 펼쳐집니다
- 각 거래를 누르면 수정, `+ 매수` / `− 매도` 로 내역 추가
- **현재가** 행을 누르면 그 종목 시세를 직접 저장할 수 있습니다
- `종목 삭제` 는 매매 내역까지 같이 정리

**엑셀 내려받기** — 설정 → `엑셀로 내려받기 (.xlsx)`.
**보유종목 / 매매내역 / 비중** 세 장짜리 진짜 xlsx 파일입니다.
라이브러리 없이 zip + XML 을 직접 만들어서 넣었습니다(`web/js/xlsx.js`).
CSV 로 하면 한글이 깨지는 일이 잦아 xlsx 로 갔습니다.

#### 차트 색

`dataviz` 스킬의 검증된 카테고리 팔레트 8색을 순서 고정으로 씁니다(돌려쓰지 않음).
9번째부터는 색을 새로 만들지 않고 "기타" 로 접습니다. 색만으로 상태를 나타내지 않도록
판정은 `!` / `✓` 기호와 수치를 항상 함께 적습니다.
...
- 폰에서 이메일·비번으로 **가입** (새 계정은 예시 데이터 없이 빈 상태로 시작합니다)
- 로그인·로그아웃은 헤더의 **사람 아이콘**에서 합니다. 로그인 중이면 초록 점이 붙습니다
...
한 기기에서 계정을 바꿔 로그인하면(다른 사람이 그 기기를 쓰는 경우) **이전 계정의
데이터를 새 계정에 섞지 않습니다.** 마지막으로 로그인했던 계정과 다르면 이 기기의
사본을 비우고 그 계정 데이터만 받아옵니다. 같은 계정으로 다시 로그인할 때는
평소처럼 병합합니다.
...
앱 말고 엑셀로 굴리고 싶을 때 쓰는 파일입니다. 저장소 루트의
**`찬홍팍_주식관리.xlsx`** 를 받아서 바로 쓰면 됩니다.

시트는 일곱 장이고, **노란 칸(파란 글씨)** 만 채우면 나머지는 전부 수식으로 나옵니다.

| 시트 | 하는 일 |
|---|---|
| 사용법 | 어디를 고치고 어디를 두는지 |
| 종목정보 | 가진 종목 등록 (티커·이름·국가·통화·섹터·현재가) |
| 거래입력 | 산 날·판 날 한 줄씩 (**날짜 순으로** 적어야 평단이 맞습니다) |
| 보유현황 | 종목별 보유수량·평단·평가금액·손익·수익률·비중 (자동) |
| 국가별 | 국가 비중과 목표 대비 판정, 조정 금액 (자동) |
| 섹터별 | 섹터 비중과 목표 대비 판정, 조정 금액 (자동) |
| 설정 | 환율, 허용오차 |

계산 규칙은 앱과 같습니다 — 평단은 이동평균(매수 수수료 포함), 팔아도 평단은
그대로 두고 실현손익만 따로, 시세가 없는 종목은 0 이 아니라 산 값으로 칩니다.

앱에 이미 넣어둔 게 있으면 **앱 → 설정 → 엑셀로 내려받기** 한 다음
`[거래내역]` 시트를 통째로 복사해서 `거래입력` 시트 A4 에 붙여넣으면 됩니다.
열 순서가 같게 맞춰뒀습니다.

```bash
python3 tools/make_excel.py                        # 파일 다시 만들기
pip install formulas
python3 tools/check_excel.py 찬홍팍_주식관리.xlsx    # 수식을 실제로 계산해 검산
python3 tools/expected_excel.py                    # 나와야 할 값(파이썬으로 따로 계산)
```

## 0-2. 구글시트로 관리하기 (자산배분 양식)

보내주신 양식(`docs/양식_원본.xlsx`)에 수식을 채운 파일입니다.
**`찬홍팍_자산배분_구글시트.xlsx`** 를 드라이브에 올리고 구글 스프레드시트로 열면 됩니다.

**초록색 칸만 채우면** 나머지는 전부 수식으로 나옵니다.

| 채우는 곳 | |
|---|---|
| 왼쪽 `B` 종목명 · `D` 매입가 · `E` 수량 | 계좌별로 뭘 얼마에 몇 주 샀는지 |
| 오른쪽 `I` 종목 · `J` 종목코드 · `W` 자산군 · `X` 수동 현재가 | 종목 통합표 |
| 위쪽 `L2:T2` 목표비율 | 자산군별 목표 |

`J` 종목코드에 `KRX:069500` / `NASDAQ:AAPL` / `NYSEARCA:SCHD` 같은 코드를 넣으면
**GOOGLEFINANCE 로 현재가·대비·등락률·갱신시간이 자동으로** 붙습니다. 구글파이낸스가
못 찾는 종목(금 현물 등)은 `X` 수동 현재가에 값을 넣으면 그 값을 씁니다.

자동으로 나오는 것: 계좌별 합계, 종목별 수량·가중평균 매입가·평가금액·현재비중,
자산군별 현재비율, **목표비중 → 목표금액 → 매매금액 → 매매량**.
`매매금액` 이 `+` 면 그만큼 더 사고, `−` 면 그만큼 팝니다.

> GOOGLEFINANCE 는 구글시트 전용입니다. 엑셀에서 열면 그 칸만 `#NAME?` 로 보입니다.

```bash
python3 tools/make_sheet_template.py                       # 양식에 수식 다시 채우기
pip install formulas
python3 tools/seed_sheet_template.py                       # 예시 숫자를 넣은 검산용 사본
python3 tools/check_sheet_template.py 검산용.xlsx           # 수식을 실제로 계산해 검산
```
