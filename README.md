...
디자인은 뉴모피즘(neumorphism) — 배경과 요소가 같은 색이고 그림자로만 도드라집니다.
라이트/다크 자동 전환.

**첫 화면** — 로그인 전에는 `웰컴 투 찬홍팍` 화면이 먼저 뜨고 거기서 바로
로그인·가입할 수 있습니다. `로그인 없이 둘러보기` 를 누르면 그냥 쓸 수 있고,
그 선택은 기억됩니다(로그아웃하면 다시 첫 화면이 뜹니다).

**현황 (폰)** — 위에서 아래로 쭉

1. **총 평가금액** + 평가손익 / 오늘 손익
2. **설정치 경고** — 국가·섹터·종목 규칙이 사실상 같은 매매를 가리키는 일이 많아
   종목 단위로 합쳐 한 줄씩 작게. 기본 4줄, 나머지는 "n건 더 보기"
3. **비중 차트** — 100% 누적 가로 막대 + 범례(이름/%/금액).
   `종목 · 국가 · 섹터` 로 전환. 9번째부터는 "기타" 로 접습니다
4. **내 종목** — 이름 / 비중, 아래 작게 보유수·평단, 오른쪽 총금액·수익률.
   **정렬: 금액순 / 수익률순 / 이름순**
5. **국가별 비중** → **섹터별 비중**

**현황 (컴퓨터, 900px 이상)** — 위쪽에 요약·경고·비중 차트를 가로로 펼치고,
아래를 2단으로 나눠 **왼쪽에 국가·섹터 현황, 오른쪽에 실제 입력한 종목 리스트**를
나란히 봅니다. 설정 화면도 2단이 됩니다.

**관리** (오른쪽 위 `내 주식 관리`) — 추가·수정·삭제를 여기서 다 합니다.

- 종목을 누르면 매매 내역이 펼쳐집니다
- 각 거래를 누르면 수정, `+ 매수` / `− 매도` 로 내역 추가
- **현재가** 행을 누르면 그 종목 시세를 직접 저장할 수 있습니다
  (자동수집이 안 되는 종목은 이렇게 넣으면 됩니다)
- `종목 삭제` 는 매매 내역까지 같이 정리
- 매수·매도 입력창의 **현재가** 버튼을 누르면 저장된 시세가 단가에 채워집니다

#### 차트 색

`dataviz` 스킬의 검증된 카테고리 팔레트 8색을 순서 고정으로 씁니다(돌려쓰지 않음).
뉴모피즘 표면(`#e6e9ef` / `#262a31`)에 대해 검증기를 돌려 라이트·다크 모두
색각 이상 분리도와 명도 대역을 통과시켰고, 대비 경고가 있는 색은 규칙대로
범례에 이름·%·금액을 항상 함께 적어 색만으로 구분하지 않게 했습니다.
