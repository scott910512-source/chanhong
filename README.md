...
**현황** — 위에서 아래로 쭉 내려보면 끝납니다.

1. **총 평가금액 한 줄** + 평가손익 / 오늘 손익
2. **설정치 경고** — 국가·섹터·종목 규칙이 사실상 같은 매매를 가리키는 경우가 많아
   종목 단위로 합쳐 **한 줄씩 작게** 보여줍니다. 기본 4줄까지, 나머지는 "n건 더 보기".
   누르면 그 종목의 관리 화면으로 바로 갑니다
3. **시세 상태 한 줄** — 예시값인지 실제 시세인지. 누르면 설정의 시세 항목으로
4. **내 종목** — 이름 / 비중, 그 아래 작게 보유수·평단, 오른쪽에 총금액·수익률
5. **국가별 비중** → **섹터별 비중** — 현재% / 목표 / 막대 / `많음·적정·적음`

**관리** (오른쪽 위 `내 주식 관리` 버튼) — 추가·수정·삭제를 여기서 다 합니다.

- 종목을 누르면 그 종목의 매매 내역이 펼쳐집니다
- 각 거래를 누르면 수정, `+ 매수` / `− 매도` 로 내역 추가
- `종목 삭제` 는 그 종목의 매매 내역까지 같이 정리합니다
- 맨 아래 `+ 새 종목 추가`
