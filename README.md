...
### 폰과 컴퓨터에서 같은 데이터 보기 (기기 동기화)

기본은 **기기 안에만** 저장됩니다. 여러 기기에서 같은 데이터를 보려면
**설정 &gt; 더보기 &gt; 기기 동기화**를 켜세요. 깃허브의 **비공개 Gist** 한 칸을
개인 저장소로 쓰고, 양쪽 기기가 그걸 읽고 씁니다. 서버를 따로 띄울 필요가 없습니다.

**처음 기기에서**
1. 깃허브 → Settings → Developer settings → Personal access tokens →
   **Tokens (classic)** → Generate new token → **gist** 만 체크
2. 앱 &gt; 설정 &gt; 기기 동기화 → 토큰 붙여넣고 **이 기기 데이터로 시작**

**두 번째 기기에서**
1. 첫 기기에서 **연결 코드 복사**
2. 두 번째 기기의 같은 화면에 **붙여넣고 연결** — 끝

이후에는 앱을 켤 때 자동으로 내려받고, 뭔가 고치면 몇 초 뒤 자동으로 올라갑니다.
...
> **알아둘 것**: 토큰은 그 기기 브라우저에만 저장되고 업로드되지 않습니다.
> 다만 비공개(secret) Gist 는 "검색에 안 뜬다"는 뜻이지 비밀번호가 걸린 게 아닙니다.
> 연결 코드에는 주소와 토큰이 들어 있으니 남에게 주지 마세요.
> 기기를 잃어버렸으면 깃허브에서 그 토큰을 폐기하면 됩니다.
...
web/                     ← 폰에서 쓰는 PWA (독립 동작, 서버 불필요)
  ...
  js/sync.js             기기 간 동기화 (깃허브 Gist) + 병합 규칙
